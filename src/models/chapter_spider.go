...
type ChapterSpider struct {
	...
	// 正常字段
	ChapterNum           int       `json:"chapterNum" gorm:"type:smallint;not null;uniqueIndex:idx_chapter_unique" spider:"chapterNum"`       // 章节编号,从1开始。组合索引
	ChapterSubNum        int       `json:"chapterSubNum" gorm:"type:smallint;not null;uniqueIndex:idx_chapter_unique" spider:"chapterSubNum"` // 章节子编号,从1/0 ?开始(从0吧，因为int默认0)。目的:如果爬的有问题，比如少几章，可以人为插入,还不影响顺序。组合索引
	ChapterRealSortNum   int       `json:"chapterRealSortNum" gorm:"type:smallint;not null;" spider:"chapterRealSortNum"`                     // 章节真实排序号-真实显示也用的它,可以是负数，负数在 有声书中，可以表示 花絮，前言、介绍等非正式语音
