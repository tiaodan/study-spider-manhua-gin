...
type Website struct {
	...
	Name                         string `gorm:"not null;check:name <> ''" `                            // 网站名称
	Domain                       string `gorm:"not null;check:domain <> ''" `                          // 域名，如：www.google.com
	...
	WebsiteType WebsiteType `gorm:"foreignKey:WebsiteTypeId;references:NameId;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT"`
}
