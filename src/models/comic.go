...
type Comic struct {
	...
	SexTypeId int `json:"sexTypeId" gorm:"not null; uniqueIndex:idx_comic_unique" ` // 总分类id-最高级-外键 组合索引字段
	...
	SexType SexType `gorm:"foreignKey:SexTypeId;references:NameId; constraint:OnUpdate:CASCADE,OnDelete:RESTRICT;"`
	...
	/*
		...
		SexTypeId int `json:"sexTypeId" gorm:"not null;column:sex_type_id"` // 总分类id-最高级-外键
		...
		SexType SexType `gorm:"foreignKey:SexTypeId; constraint:OnUpdate:CASCADE,OnDelete:RESTRICT;"`
