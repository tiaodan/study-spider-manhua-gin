...
type ChapterMy struct {
	...
	ChapterNum           int       `json:"chapter_num" gorm:"not null;uniqueIndex:idx_comic_unique" spider:"chapterNum"`        // 章节编号,从1开始。组合索引
	ChapterSubNum        int       `json:"chapter_sub_num" gorm:"not null;uniqueIndex:idx_comic_unique" spider:"chapterSubNum"` // 章节子编号,从1/0 ?开始(从0吧，因为int默认0)。目的:如果爬的有问题，比如少几章，可以人为插入,还不影响顺序。组合索引
