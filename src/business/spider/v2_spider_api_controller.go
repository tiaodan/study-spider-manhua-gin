...
func (api *ConfigDrivenAPIController) DispatchApi_OneTypeAllBookByHtml_ConfigDriven(c *gin.Context) {
	...
	if config.Crawl.Selectors != nil {
		if bookContainer, exists := config.Crawl.Selectors["book_container"]; exists {
			// bookArrCssSelector: 容器选择器，包含所有书籍项目的父元素
			// 对于kxmanhua，使用.product-page作为容器选择器
			params["bookArrCssSelector"] = ".product-page"
			// bookArrItemCssSelector: 单个项目选择器
			params["bookArrItemCssSelector"] = bookContainer
		}
	}

	log.Infof("开始配置驱动爬取: website=%s, urls=%d", website, len(urls))
