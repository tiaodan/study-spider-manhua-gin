...
# v0.0.0.20.5 临时提交
    - 区分自己的目的 √
        - 只要韩漫？ -- 我的系统是只要韩漫 √
        - 韩漫,日漫都要???? -- comic_spider 按照网站的分类进行区分 √
        - 想到了，我的系统只要韩漫，但是网站可能分类不太对，comic_spider就是按照网站的分类，爬到库里。我用的时候再根据实际修改呀！√
    - 区分要爬哪几个网站。目标是选出来，前3要用的网站。要求:1.分类清晰 2没有多余水印(除了toptoon) 3 爬取简单，没有防爬机制 4. 图片内容不会过期 5. 不用翻墙 -》 level1
        - 自己找韩漫 google :
            - bkmanhua(布卡漫画) level1 (韩漫、日漫分的不清楚，没有网站水印，也没有被爬取的其它网站水印) https://bkmanhua.com
            - rouman8.com (肉漫吧) level1 (韩漫、日漫分清楚，没有网站水印，也没有被爬取的其它网站水印) https://rouman8.com
            - toptoon.net level1 (韩漫分的清楚，没有网站水印，担有限制，可以作为主参考) https://toptoon.net
            - jjmhw.cc() level1 (韩漫、日漫分还算清楚-网页版清楚、手机版不清楚，没有网站水印，也没有被爬取的其它网站水印，不用翻墙))
            - comicbox.xyz(污污漫画) level2 (韩漫、日漫分清楚，不知道有没有水印，不好搜索，只能搜繁体字，还得翻墙)
            - Asia comics.com level3 (韩漫、日漫分的不清楚, 不知道有没有水印,"协议换爱"搜不到，得翻墙)
        porndude推荐:
        - kxmanhua(开心看漫画) 考虑，但是有水印 level: 2 (好用，但不推荐，有水印)
        - nnhanman.xyz(鸟鸟漫画) 考虑 level: 1.5 (韩漫、日漫分的不清楚，没水印)
        - 55comics(55漫画) level2 (简体/繁体中文搜不到内容：比如:协议换爱，分类还可以)
        - kmh991.com(K漫画) level3 (界面看不清，有没有水印，不清楚)
        - hmzxa.com(韩漫在线) level4 (手机广告太多, 根本点不到分类按钮)
    - 区分应如何分类:
        - 如果一个网站有很多tag，如:大尺度、巨乳、人妻等,自己不好分类的时候，插入comic_spider 就先按 "待分类"算
        - 分类: 韩漫, 日漫, 欧美漫、 3D漫画, 真人漫画, 耽美(同性)、同人志(根据现有漫画改编)、单行本(出版漫画-大概率日本) √
        - 还分黑白漫画、彩色漫画(大部分韩漫是彩色的) - 这2个品类，几乎包含了所有分类了，计划放到tag里
    - 先处理：爬取某个网站所有书                
        - 先都爬完
        - 看能否只区分网站，做一个比较通用的模板
    - 再处理：爬取某个书所有章节
    ...
    - 根据这几个网站：定义通用type分类 √
        - 定义完了：韩漫, 日漫, 欧美漫、 3D漫画, 真人漫画, 耽美(同性)、同人志(根据现有漫画改编)、单行本(出版漫画-大概率日本) √
        - 其它都放到tag表里 √

    - 实现: 传真实网站url后, 完整逻辑 ？？
        - 爬某一类所有书
            - 传参 分类 带%的url，和endNum。比如：kxmanhua: https://kxmanhua.com/manga/library?type=2&complete=2&page=%d&orderby=1 -> 把%d 替换endNum[i]
        - 爬1本书所有章节
        - 爬1本章节所有章节内容
    - 实现，可以根据url，指定爬一本书。不是按分类url爬所有
    - 实现, 可以自己下载并上传漫画，就不过comic_spider了，直接到comic_my库

# v0.0.0.20.6 临时提交
    - 修改V2 通用爬虫框架前，实现爬取1页book，但只能插入comic_spider库，不能插入comic_stats表+author表

# v0.0.0.20.7 临时提交 --------------- 清理代码还没实现
    - 主要改动：提交以下，AI写的太垃圾了，结构根本是混乱的。我考虑用1.5版本修改，就是根据1.0自己写的代码修改
    - AI 修改太多无用代码，也把之前代码改乱了，我要重新整理 -》 没实现
    - 经验：代码不能完全给不靠谱的AI托管，会越改越乱。靠谱的我看就cursor
    - 经验：AI只能处理小部分需求，比如一个小方法，方法里一块小逻辑，做的好，完全业务，绝对不能给AI托管，自己搭建框架，让AI 往里填小肉



# v0.0.0.21
版本总结:
    - 可以给一个方法 后缀起名funcV1 funcV2 -> 表示V1版算法/V2版算法。这样就不用在方法里写 很多文档注释，区分v0.1 v0.2
    ...
    - 爬取目标
        ...
        - 其余9个网站都爬完
    - 考虑加一个tag表（只有my，没有spider表），还有一个 comic_my_tag_relatino表，手动修改tag的时候，插入里面
    - 思考赚钱方式,能不能长久
...
- 重要的是提升信息，什么能提升？
    - 成果
    - 如何有成果?就是最小成果法
        - 实现最小成果-流程跑通。1个网站-》1页book ->2个章节 -》2个章节内容 -》上传到aws -》上传到baidu -》 手机实现
            - 1个网站
                - 1页book
                    - book √
                    - author x 这个网站没有相关参数，代码无法测试
                    - book-stats
        - 看数据对不对
        - 升级
...
- 考虑app埋点，分析数据
- 第 N 话 , 爬取的时候, 需要去除空格
- 爬取，如果爬到的总数是0，不要修改父类状态，不要修改父表数据
...
# v0.1.5.0 临时2 
    总结：爬韩漫所有时，comic_spider 和comic_spider_stats 数据不一致，先提交
...
# v0.1.5.0 临时4
    ...
    onetype
        ...
        - 把chapter_spider \ chapter_my 结构体里 parentId提到最前面，因为是唯一索引，这样看得清楚 √
        - 尝试爬取kxmanhua 网站所有数据，保存一类，导出一个sql，再导入其它库，测试sql能用。保存爬取数据，作为备份 √
            - 爬韩漫 alltype √
            - 爬韩漫 allbook
                - 1 能爬已有的 社团学姐, 总个数，和实际插入数量 能对上 √
                - 2 能爬本地 1.html，解决报错, 总个数，和实际插入数量 能对上 √ 
                - 3 爬爬取逻辑改成配置项形式，试验社团学姐 + 1.html √
                - 4 排查 +不加 update_at 区别 -------------------------------  -》 要想upsert 更新，必须加上upated_at √
                - 知道了, 如果报错：DB更新失败: 在模型中找不到字段 ParentId。一般是因为： √
...
# v0.1.5.0 临时8
- 更新第几话逻辑，能识别中文汉字，但是实际不行，没定位原因 ！

# v0.1.5.0 临时9
    总结: 自动并手动爬完 kxmanhua 所有内容
                - 爬取 外传-第几话，生成的号码不对，应该按第X话，提取+ -10 √
                - stats表，统计总数逻辑需要改下，应该不包括 前言、后记、试听这些 √ 改了逻辑，验证通过 √
...
# v0.1.5.0 临时12
    总结：修复chapter_content url为空
    1. 1949 chapter 这些插入报错的，要重新尝试插入 √
        重新插入报错数据：4060 3222 3149 2200 1988 1949 1887 1888 1876 1869 1868 872 871 873 869 866 868 865 864 862 863 809 807 806
        chapter 本来就没有内容：
        [ 4064    ]
...
# v0.1.5.0 临时14
    总结: 找到不并发原因。colly 没设置 Parallelism
    - 爬到全是空的，为啥报异常，就不往下走了 -> SpiderManyChapterAllContent_UpsertPart 函数，加了判断数组空就跳过逻辑  √
...
# v0.1.5.0 临时18
    总结：chapter_content_spide分表，uthor 关联表字段是否加 websiteID
    考虑 分表 ，和author 关联表字段是否加 websiteID
        - 数据迁移的时候，创建分表 √
        - 新建website的时候，调用数据迁移，创建相关表 √
        - website 没有dataclean √
...
版本总结: 要一劳永逸，代码不变，改配置就能实现需求
    ...
    onetype
        ...
        - oneType方法, 区分website case需要统一下 √
            - DispatchApi_SpiderOneTypeAllBookArr_V1_5_V2 // 不带switch case，把kxmanhua 里的逻辑全提到外面来
            - DispatchApi_SpiderOneTypeAllBookArr_V1_5_V1 // 带switch case 区分不通网站的 爬取逻辑
        - 遇到问题：插入韩漫-完结，韩漫-连载后，comic_spider表，比comic_spider_stats表多12个 √
            - 排查是因为有相同数据，导致charstats表执行了update操作。我早该想到的 √
            - 解决办法：插入stats表前，查询comicId的条件少，要增加 processId。应该覆盖所有索引值啊 √ 
        - 排查，爬多个页面，慢的原因，统计大概几秒，log 看这个请求多长时间返回,性能分析 √
            - 主要问题在于，colly 设置的 "模拟用户操作的 时间延迟" √
        ...
        - 尝试爬取kxmanhua 网站所有数据，保存一类，导出一个sql，再导入其它库，测试sql能用。保存爬取数据，作为备份 √
            ...
            - 爬韩漫 allbook
                ...
                明日目标：爬完所有chapterContent,
                        ...
                        - 把v1_spiderMappping.go 单独拿出来，把mapping相关内容单独拿出来 ？？
                        - 创建一些error 错误备用，应该写出单例吗？ 不用
                        - // 如果子表爬到数据了，才处理 没实现--
